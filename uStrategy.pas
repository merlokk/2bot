...
begin
 ...
 begin
   ...
   // проверка на то, что планета не появилась
...
begin
  ...
  begin
    ...
    begin
      ...
      if (pl = nil) or (pl.Name <> 'Планета') or
...
begin
  ...
  try
    ...
    SetLength(Fleet, 1);
    Fleet[0].Name := 'Солнечный спутник';
...
begin
  ...
  try
    ...
    SetLength(Fleet, 1);
    Fleet[0].Name := 'Сборщик Тёмной материи';
...
begin
  ...
  try
    ...
    pl := FImperium.GetPlanet(FFromPlanetID);
    if (pl = nil) or
       (pl.GetBuildLevel('Исследовательская лаборатория') < 1)
    then exit;
